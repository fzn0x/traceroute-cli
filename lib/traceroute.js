...
internals.Traceroute.trace = function (host, callback) {
  dns.lookup(host.toUpperCase(), (err) => {
    ...
    const command = internals.isWin ? "tracert -d " : "traceroute -q 1 -n ";

    const child = spawn(command, [host]);
    ...
    child.on("close", function () {
      const results = internals.parseOutput(scriptOutput);
